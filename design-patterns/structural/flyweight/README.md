...
Large numbers of similar objects duplicate common data.

In small programs you may not need Flyweight. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Text editor character styles, map markers, game particles.
...
`src/main/java/com/anish/lld/patterns/structural/FlyweightDemo.java`
...
"Flyweight is useful when share immutable intrinsic state to reduce memory usage. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `MarkerIconFactory` is the center of the pattern.

Design pressure:

Thousands of markers can share the same icon object.
...
Add cache stats and eviction for rarely used icons.

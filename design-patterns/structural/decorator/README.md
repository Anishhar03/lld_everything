...
Add behavior dynamically by wrapping an object.

## Why This Pattern Exists

Subclassing for every feature combination causes class explosion.

In small programs you may not need Decorator. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Coffee add-ons, input streams, middleware.
...
`src/main/java/com/anish/lld/patterns/structural/DecoratorDemo.java`
...
"Decorator is useful when add behavior dynamically by wrapping an object. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `CoffeeDecorator` is the center of the pattern.

Design pressure:

Add optional features without subclassing every combination.
...
Add SugarDecorator and WhippedCreamDecorator.

...
Direct access needs caching, authorization, lazy loading, or remote behavior.

In small programs you may not need Proxy. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Document download proxy, image lazy loading proxy, secure service proxy.
...
`src/main/java/com/anish/lld/patterns/structural/ProxyDemo.java`
...
Proxy can hide latency or authorization behavior if not documented.

## Interview Explanation

"Proxy is useful when control access to another object. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `SecureReportProxy` is the center of the pattern.

Design pressure:

Access to report download needs authorization before real object is called.
...
Add caching and audit logging in the proxy.

...
Tree structures require different handling for leaves and containers.

In small programs you may not need Composite. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.
...
`src/main/java/com/anish/lld/patterns/structural/CompositeDemo.java`
...
Can make invalid operations available on leaf nodes if the interface is too broad.

## Interview Explanation

"Composite is useful when treat individual objects and groups of objects uniformly. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `FileSystemNode` is the center of the pattern.

Design pressure:

Files and folders should be handled uniformly in a tree.
...
Add delete/search operation across the tree.

...
A third-party or legacy class has useful behavior but an incompatible interface.

In small programs you may not need Adapter. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Adapting legacy payment API to a new PaymentProcessor interface.
...
`src/main/java/com/anish/lld/patterns/structural/AdapterDemo.java`
...
Too many adapters can hide inconsistent domain modeling.

## Interview Explanation

"Adapter is useful when convert one interface into another expected by client code. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `LegacyPaymentAdapter` is the center of the pattern.

Design pressure:

Legacy payment API speaks in paise but the app expects rupees.
...
Adapt a second payment gateway with a different API.

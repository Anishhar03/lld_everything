...
Undo/restore is needed but object internals should remain encapsulated.

In small programs you may not need Memento. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Text editor undo, game save, form draft snapshot.
...
`src/main/java/com/anish/lld/patterns/behavioral/MementoDemo.java`
...
"Memento is useful when capture and restore object state without exposing internals. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `EditorSnapshot` is the center of the pattern.

Design pressure:

Editor supports undo while keeping internal state private.

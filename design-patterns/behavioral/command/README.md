...
You need queueing, retry, logging, undo, or decoupling between invoker and receiver.

In small programs you may not need Command. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Text editor commands, job queue tasks, remote control buttons.
...
`src/main/java/com/anish/lld/patterns/behavioral/CommandDemo.java`
...
Creates many small command classes.

## Interview Explanation

"Command is useful when encapsulate a request as an object. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `LightOnCommand` is the center of the pattern.

Design pressure:

A button should execute/undo actions without knowing receiver details.

...
Notify subscribers when an object changes state.

## Why This Pattern Exists

One object changes and many independent objects need to react.

In small programs you may not need Observer. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Order status notifications, stock alerts, event listeners.
...
`src/main/java/com/anish/lld/patterns/behavioral/ObserverDemo.java`
...
Can create unexpected cascades or memory leaks if subscribers are not removed.

## Interview Explanation

"Observer is useful when notify subscribers when an object changes state. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `EventPublisher` is the center of the pattern.

Design pressure:

Many subscribers react when one event is published.
...
Add unsubscribe and async notification.

...
Pass a request through a chain of handlers until handled.

## Why This Pattern Exists

Validation or processing has multiple ordered steps that should be independently extensible.

In small programs you may not need Chain Of Responsibility. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Authentication, authorization, validation, logging pipeline.
...
`src/main/java/com/anish/lld/patterns/behavioral/ChainOfResponsibilityDemo.java`
...
Request flow can become hard to trace if chain order is unclear.

## Interview Explanation

"Chain Of Responsibility is useful when pass a request through a chain of handlers until handled. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `RequestHandler` is the center of the pattern.

Design pressure:

Request passes through auth, validation, and other independent handlers.
...
Add RateLimitHandler in the chain.
...
"I would use Chain Of Responsibility here because the requirement has a clear variation point. By moving that variation behind an abstraction, the stable workflow remains unchanged and new behavior can be added in a focused class. The tradeoff is extra classes, so I would use it only when this part is expected to grow or change."

### One-Line Revision

Chain Of Responsibility = solve the changing-design pressure cleanly, without forcing client code to know every concrete detail.

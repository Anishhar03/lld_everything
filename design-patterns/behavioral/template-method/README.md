...
Define an algorithm skeleton while subclasses customize steps.

## Why This Pattern Exists

Several workflows share the same high-level order but differ in some steps.

In small programs you may not need Template Method. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Report generation, data import, build pipeline.
...
`src/main/java/com/anish/lld/patterns/behavioral/TemplateMethodDemo.java`
...
Inheritance-based; can be less flexible than composition.

## Interview Explanation

"Template Method is useful when define an algorithm skeleton while subclasses customize steps. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `DataImporter` is the center of the pattern.

Design pressure:

Import workflow has fixed steps but each source customizes details.

...
State-specific if-else logic spreads through the class.

In small programs you may not need State. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Vending machine, order lifecycle, document workflow.
...
`src/main/java/com/anish/lld/patterns/behavioral/StateDemo.java`
...
More classes; state transitions must be easy to trace.

## Interview Explanation

"State is useful when let an object change behavior when its internal state changes. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `OrderState` is the center of the pattern.

Design pressure:

Order behavior changes based on lifecycle state.
...
Add CancelledState with valid/invalid transitions.

...
Centralize communication among many objects.

## Why This Pattern Exists

Many objects directly reference each other, creating a tangled graph.

In small programs you may not need Mediator. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Chat room, UI form coordinator, air traffic controller.
...
`src/main/java/com/anish/lld/patterns/behavioral/MediatorDemo.java`
...
Mediator can become too large if it absorbs all behavior.

## Interview Explanation

"Mediator is useful when centralize communication among many objects. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `ChatRoom` is the center of the pattern.

Design pressure:

Users communicate through a room instead of referencing every other user.

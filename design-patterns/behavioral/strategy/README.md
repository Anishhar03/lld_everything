...
Multiple algorithms exist and client code should not be full of if-else blocks.

In small programs you may not need Strategy. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Pricing, payment, route calculation, sorting.
...
`src/main/java/com/anish/lld/patterns/behavioral/StrategyDemo.java`
...
Creates separate classes for each algorithm.

## Interview Explanation

"Strategy is useful when select an algorithm at runtime. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `PricingStrategy` is the center of the pattern.

Design pressure:

Pricing algorithm changes without changing Cart.
...
Add CouponPricing and MembershipPricing.

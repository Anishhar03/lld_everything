...
Client code should not know if data is stored as array, list, tree, or paginated source.

In small programs you may not need Iterator. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Playlist iterator, custom collection iterator.
...
`src/main/java/com/anish/lld/patterns/behavioral/IteratorDemo.java`
...
Concurrent modification behavior must be clear.

## Interview Explanation

"Iterator is useful when traverse a collection without exposing its internal representation. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `Playlist iterator` is the center of the pattern.

Design pressure:

Client should traverse songs without knowing storage structure.

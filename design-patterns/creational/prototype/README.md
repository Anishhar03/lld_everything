...
Object creation is expensive or starts from a common template.

In small programs you may not need Prototype. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Document templates, game enemies, predefined notification templates.
...
`src/main/java/com/anish/lld/patterns/creational/PrototypeDemo.java`
...
"Prototype is useful when create new objects by copying existing objects. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `DocumentTemplate` is the center of the pattern.

Design pressure:

New objects start from a reusable template instead of rebuilding common state.
...
Add deep-copy support for mutable child sections.

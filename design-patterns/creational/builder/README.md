...
Build complex objects step by step with readable construction and validation.

## Why This Pattern Exists

Constructors with many optional parameters become unreadable and error-prone.

In small programs you may not need Builder. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

HTTP request, user profile, immutable configuration.
...
`src/main/java/com/anish/lld/patterns/creational/BuilderDemo.java`
...
More code than constructors; best for complex objects.

## Interview Explanation

"Builder is useful when build complex objects step by step with readable construction and validation. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `HttpRequest.Builder` is the center of the pattern.

Design pressure:

Object has many optional fields and needs validation before creation.
...
Add headers and query params while keeping construction readable.

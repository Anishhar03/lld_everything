...
Create objects without exposing concrete creation logic to client code.

## Why This Pattern Exists

Client code uses if-else or switch statements to decide which concrete class to instantiate.

In small programs you may not need Factory Method. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Create EmailNotification, SmsNotification, or PushNotification based on input.
...
`src/main/java/com/anish/lld/patterns/creational/FactoryMethodDemo.java`
...
Adds an extra factory layer; useful only when creation varies.

## Interview Explanation

"Factory Method is useful when create objects without exposing concrete creation logic to client code. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `NotificationFactory` is the center of the pattern.

Design pressure:

Notification type comes from request/config and client should not instantiate concrete classes.
...
Add PushNotification without changing caller code.

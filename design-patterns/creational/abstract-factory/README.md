...
Create families of related objects without depending on concrete classes.

## Why This Pattern Exists

Objects must be compatible with a chosen family, such as Windows UI vs Mac UI.

In small programs you may not need Abstract Factory. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Create Button and Checkbox families for different platforms.
...
`src/main/java/com/anish/lld/patterns/creational/AbstractFactoryDemo.java`
...
"Abstract Factory is useful when create families of related objects without depending on concrete classes. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `UiFactory` is the center of the pattern.

Design pressure:

A product family must stay consistent, such as Mac button with Mac checkbox.
...
Add LinuxFactory with LinuxButton and LinuxCheckbox.
...
"I would use Abstract Factory here because the requirement has a clear variation point. By moving that variation behind an abstraction, the stable workflow remains unchanged and new behavior can be added in a focused class. The tradeoff is extra classes, so I would use it only when this part is expected to grow or change."

### One-Line Revision

Abstract Factory = solve the changing-design pressure cleanly, without forcing client code to know every concrete detail.

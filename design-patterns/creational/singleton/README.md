...
Some objects represent shared process-level state, such as application configuration. Creating many instances can cause inconsistent state or wasted resources.

In small programs you may not need Singleton. In LLD interviews, the pattern becomes useful when requirements start changing and direct code would become brittle.

## Real-World Analogy

Configuration provider, metrics registry, application-wide logger.
...
`src/main/java/com/anish/lld/patterns/creational/SingletonDemo.java`
...
Can become hidden global state and make testing harder if abused.

## Interview Explanation

"Singleton is useful when ensure a class has only one instance and provide a global access point. In my design I would use it when the changing part should be isolated from the stable client code. The tradeoff is that it introduces extra classes, so I would use it only when extension or decoupling is actually needed."
...
In the demo implementation, `ConfigurationRegistry` is the center of the pattern.

Design pressure:

Only one configuration registry should exist in the application.
...
Add a test hook to reset instance only in tests.

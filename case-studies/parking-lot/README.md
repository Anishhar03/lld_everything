...
Design a parking lot with multiple floors, spot types, vehicle types, entry gates, exit gates, tickets, pricing, payment, and spot allocation.
...
- Strategy for spot allocation and pricing
- Factory for vehicle/spot creation
- Repository for floor and ticket storage
...
The shared resource is a parking spot. The check for availability and marking occupied must be atomic. In memory, synchronize the spot manager. In production, use database transactions or row-level locking.
...
Spot assignment is a strategy because it can change:

- nearest spot first
- cheapest floor first
- EV spot preference
- large vehicle allocation
- reserved parking
...
Finding a free spot and marking it occupied must happen in one atomic operation.

...
Design Payment System at high level.
...
If the same key is retried, return the original payment result.

### Double Callback Problem

Payment gateways may send duplicate callbacks.

Solution:

- Store gateway event id.
- Ignore duplicate processed events.
- Use transaction to update payment status.

### Ledger

For serious payment systems, use append-only ledger entries instead of only updating balances.

```text
LedgerEntry(id, account_id, debit, credit, reference_id, created_at)
```

### Consistency

Payments need strong consistency for money movement. Analytics, emails, and notifications can be async.

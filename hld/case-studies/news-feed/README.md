...
Design News Feed at high level.
...
When user posts, push post ID to followers' feed inboxes.
...
When user opens feed, fetch posts from followed users and rank.
...
```mermaid
flowchart LR
    PostService --> FanoutWorker
    FanoutWorker --> FeedCache[(Feed Cache)]
    CelebrityPosts[(Celebrity Post Store)] --> FeedService
...
Feed can be eventually consistent. Post creation should be strongly persisted.

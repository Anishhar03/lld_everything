...
Design Chat System at high level.
...
Chat needs persistent connections.

Use WebSocket servers:

```mermaid
flowchart LR
    UserA --> WS1[WebSocket Server 1]
    UserB --> WS2[WebSocket Server 2]
    WS1 --> Presence[(Presence Store)]
    WS2 --> Presence
    WS1 --> MQ[(Message Broker)]
    MQ --> WS2
    WS1 --> DB[(Message DB)]
```
...
2. Server authenticates and validates.
3. Message is persisted.
4. Event is published to broker.
5. Recipient's WebSocket server receives event.
6. Message is delivered if user is online.
7. If offline, push notification may be sent.

### Ordering

For one conversation, order messages by monotonically increasing sequence number.
...
Use wide-column or partitioned SQL table:

```text
Message(conversation_id, sequence_id, sender_id, body, created_at)
```
